...
function UpdateProfileCustomer()
{
	...
	$newemail=$_POST['newemail'];
		...
		$con=mysqli_connect("localhost","g01s41","12345","g01s41db");
		$sql = "update customer SET password='$newpassword', email='$newemail' WHERE username='$username' && email='$email'";
	    $qry=mysqli_query($con,$sql);
		echo "<script>";
	    echo " alert('Password and Email have been changed');
		  </script>";
		header( "refresh:1; url=viewprofilecustomer.php" );
		}else
	    {
		echo "<script>";
	    echo " alert('Password and Confirm Password Are Not Same OR  Email Wrong!!!');
		  </script>";
		 header( "refresh:1; url=editprofilecustomer.php" ); 	 
		}
